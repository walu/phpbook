...
````
第一种就是我们处理PHP语言中字符串索引数组时使用的，第二种是我们处理PHP语言中数字索引数组使用的。回忆下第二章，当一个数据被添加到HashTable时，一个新的存储块分配给它，并且数据被复制到其中；当数据被提取时将返回指向数据的指针。下面的代码是添加data1到HashTable的代码片段，并且在程序的结尾提取它，*data2的数据将与*data1的数据一致，但是它们的指针所指向的内存地址不同。
````c
...
这个函数又繁琐了些，与zend_hash_copy相比，其多了两个参数，多出来的pMergeSoure回调函数允许我们选择性的进行merge，而不是全都merge。merge_checker_func_t pMergeSource, void *pParam);允许使用一个合并检查函数有选择的进行拷贝，以下的例子显示了zend_hash_merge_ex()仅拷贝源HashTable(正好是用户空间变量数组)中关联索引成员；
````c
...
<div class="tip-common">这个函数通过C语言中的可变参数特性来接收参数。这个特殊的例子不需要传递参数; 对于从va_list args中提取可变参数, 请参考POSIX文档的va_start(), va_arg(), va_end()。</div>
...
在我们遍历一个HashTable时，一般是很难陷入死循环的。在遍历HashTable时, 尤其是当它包含用户空间变量时, 少数情况下会碰到循环引用或者说自交的循环. 如果一个迭代上下文的循环开始后, HashTable的内部指针被调整, 接着内部启动了对同一个HashTable的迭代循环, 它就会擦掉原有的当前内部指针位置, 内部的迭代将导致外部的迭代被异常终止。

对于使用zend_hash_apply样式的实现以及自定义的向前移动的用法, 均可以通过外部的HashPosition变量的方式来解决这个问题。

前面列出的zend_hash_*()函数均有对应的zend_hash_*_ex()实现, 它们可以接受一个HashPosition类型的参数. 因为HashPosition变量很少在短生命周期的循环之外使用, 因此将它定义为直接变量就足够了. 接着可以取地址进行使用, 如同下面的你曾经见过的php_sample_print_var_hash()函数：
...
（上面的例子中，不知为何无法运行，应该是zend_hash_minmax传递给fname_compare的两个参数类型是Bucket **，而不是zend_function，大家可以试着用将参数类型改为Bucket **，然后使用(zend_function *)(*a)->pData;来转换实验下）
...
这个函数首先会比较两个HashTable的元素个数. 如果其中一个元素个数多于另外一个, 则直接认为它比另外一个大, 快速返回。如果两者的元素一样多，然后就比较它们各自的第一个元素，接下来, 循环遍历hta. 如果设置了ordered标记, 它将hta的第一个元素和htb的第一个元素的key长度进行比较, 接着使用memcmp()二进制安全的比较key内容. 如果key相等, 则使用提供的compar回调函数比较两个元素的值.
